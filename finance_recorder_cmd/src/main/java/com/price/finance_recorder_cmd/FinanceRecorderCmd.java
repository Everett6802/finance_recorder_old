...
public class FinanceRecorderCmd
{
	...
	private static String get_time_elapse_string(long time_start_millisecond, long time_end_millisecond)
	{
		...
		if (time_lapse_millisecond >= (100 * 1000))
			...
			if (time_lapse_millisecond >= (10 * 1000))
				time_lapse_msg = String.format("######### Time Lapse: %d second(s) #########", (int) ((time_end_millisecond - time_start_millisecond) / 1000));
			else
				time_lapse_msg = String.format("######### Time Lapse: %d second(s) #########", (int) ((time_end_millisecond - time_start_millisecond) / 1000));
	...
	private static void show_usage_and_exit()
	{
		try
		{
			BufferedReader br = new BufferedReader(new InputStreamReader(FinanceRecorder.class.getClassLoader().getResourceAsStream("help.txt")));
			String line = null;
	...
	private static short parse_param(String args[], boolean early_parse_param)
	{
		...
		while (index < args_len)
		{
			...
			if (option.equals("--market_mode")) // Need to be parsed first
			{
				...
				if (option.equals("--stock_mode")) // Need to be parsed first
				{
					if (early_parse_param)
					{
					...
					if (option.equals("--silent"))
					{
						...
						if (option.equals("-h") || option.equals("--help"))
						{
							...
							if (option.equals("--company_profile_filepath"))
							{
								if (!early_parse_param)
								{
								...
								if (option.equals("--compare_company"))
								{
									if (!early_parse_param)
									{
									...
									if (option.equals("--renew_company"))
									{
										if (!early_parse_param)
										{
										...
										if (option.equals("--statement_profile_filepath"))
										{
											if (!early_parse_param)
											{
											...
											if (option.equals("--compare_statement"))
											{
												if (!early_parse_param)
												{
												...
												if (option.equals("--renew_statement"))
												{
													if (!early_parse_param)
													{
													...
													if (option.equals("--finance_folderpath"))
													{
														if (!early_parse_param)
														{
															if ((index + 1) >= args_len)
														...
														if (option.equals("--finance_backup_folderpath"))
														{
															if (!early_parse_param)
															{
																if ((index + 1) >= args_len)
															...
															if (option.equals("--finance_restore_folderpath"))
															{
																if (!early_parse_param)
																{
																	if ((index + 1) >= args_len)
															...
															// else if
															// (option.equals("--finance_backup_foldername"))
															// {
															// if (!early_parse_param)
															// {
															// if (index + 1 >= args_len)
															// show_error_and_exit(String.format("The
															// option[%s] does NOT
															// contain value", option));
															// finance_backup_foldername_param
															// = args[index + 1];
															// }
															// index_offset = 2;
															// }
															// else if
															// (option.equals("--finance_restore_foldername"))
															// {
															// if (!early_parse_param)
															// {
															// if (index + 1 >= args_len)
															...
															// option[%s] does NOT
															// contain value", option));
															// finance_restore_foldername_param
															// = args[index + 1];
															// }
															// index_offset = 2;
															// }
															else
																if (option.equals("--delete_sql_accurancy"))
																{
																	if (!early_parse_param)
																	{
																		if ((index + 1) >= args_len)
																	...
																	// else if
																	// (option.equals("--show_finance_backup_list"))
																	// {
																	// if (!early_parse_param)
																	// {
																	// show_finance_backup_list_param
																	// = true;
																	// }
																	// index_offset = 1;
																	// }
																	// else if
																	// (option.equals("--show_finance_restore_list"))
																	// {
																	// if (!early_parse_param)
																	// {
																	// show_finance_restore_list_param
																	// = true;
																	// }
																	// index_offset = 1;
																	// }
																	else
																		if (option.equals("--set_operation_non_stop"))
																		{
																			if (!early_parse_param)
																			{
																				if ((index + 1) >= args_len)
																			...
																			if (option.equals("-o") || option.equals("--operation"))
																			{
																				if (!early_parse_param)
																				{
																					if ((index + 1) >= args_len)
																				...
																				if (option.equals("--method_from_file"))
																				{
																					if (!early_parse_param)
																					{
																						if ((index + 1) >= args_len)
																					...
																					if (option.equals("-m") || option.equals("--method"))
																					{
																						if (!early_parse_param)
																						{
																							if ((index + 1) >= args_len)
																						...
																						if (option.equals("-t") || option.equals("--time_range"))
																						{
																							if (!early_parse_param)
																							{
																								if ((index + 1) >= args_len)
																							...
																							if (option.equals("--company_from_file"))
																							{
																								if (!early_parse_param)
																								{
																									if ((index + 1) >= args_len)
																								...
																								if (option.equals("-c") || option.equals("--company"))
																								{
																									if (!early_parse_param)
																									{
																										if ((index + 1) >= args_len)
	...
	private static short check_param()
	{
		...
		if (set_operation_non_stop_param != null)
		{
			if ((set_operation_non_stop_param.equals("TRUE") | set_operation_non_stop_param.equals("True")) || set_operation_non_stop_param.equals("true"))
			{
				...
				if (set_operation_non_stop_param.equals("FALSE") || set_operation_non_stop_param.equals("False") || set_operation_non_stop_param.equals("false"))
				{
