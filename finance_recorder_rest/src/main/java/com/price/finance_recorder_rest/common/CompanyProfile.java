...
{
	final static Logger logger = Logger.getLogger(CompanyProfile.class);
	...
	private static class CompanyProfileEntry implements Comparable<CompanyProfileEntry>
	{
		...
		public void add(final String new_element)
		{
			profile_element_array.add(new_element);
		}
	...
	private static String dataset_absolute_folderpath = null;
	private static CompanyProfile instance = null;
	public static CompanyProfile get_instance(String dataset_path)
	{
		if (instance == null)
			allocate(dataset_path);
		return instance;
	}
// For thread-safe
	private static synchronized void allocate(String dataset_path)
	{
		String dataset_absolute_path = CmnFunc.get_dataset_absolute_path(dataset_path);
		if (!dataset_absolute_path.equals(dataset_absolute_folderpath))
			instance = null;
		if (instance == null)
		{
	...
	String conf_folderpath = null;

	private short initialize()
	{
		assert dataset_absolute_folderpath != null : "dataset_absolute_folderpath should NOT be null";
		short ret = CmnDef.RET_SUCCESS;
		conf_folderpath = String.format("%s/%s", dataset_absolute_folderpath, CmnDef.CONF_FOLDERNAME);
	...
	private short parse_company_profile_conf()
	{
		...
		ret = CmnFunc.read_file_lines(CmnDef.COMPANY_PROFILE_CONF_FILENAME, conf_folderpath, config_line_list);
		if (CmnDef.CheckFailure(ret))
			return ret;
		OUT : for (String line : config_line_list)
		{
			...
			{
				logger.error(String.format("Incorrect config format: %s", line));
	...
	private short parse_company_group_conf()
	{
		...
		short ret = CmnDef.RET_SUCCESS;
		LinkedList<String> config_line_list = new LinkedList<String>();
		ret = CmnFunc.read_file_lines(CmnDef.COMPANY_GROUP_CONF_FILENAME, conf_folderpath, config_line_list);
		if (CmnDef.CheckFailure(ret))
			return ret;
		int line_cnt = 0;
		...
		OUT : for (String line : config_line_list)
		{
			...
			{
				logger.error(String.format("Incorrect company group number, expected: %d, actual: %d", line_cnt, Integer.valueOf(group_number)));
	...
	private short generate_company_profile_sorted_list()
	{
		...
		// for (CompanyProfileEntry company_profile_entry :
		// company_profile_sorted_list)
	...
	{
		if (company_profile_sorted_list == null)
			init_company_profile_sorted_deque();
	...
	{
		if (company_group_profile_sorted_list == null)
			init_company_group_profile_sorted_deque();
		return new TraverseEntry(company_group_profile_sorted_list.get(index));
	}

	public int get_company_group_size()
	{
	...
	public Integer lookup_company_group_number(String company_number)
	{
		return Integer.valueOf(lookup_company_profile(company_number).get(COMPANY_PROFILE_ENTRY_FIELD_INDEX_GROUP_NUMBER));
	}

	public String get_dataset_company_folderpath(String company_number, int company_group_number, String dataset_path)
	{
		return String.format(CmnFunc.get_dataset_stock_company_data_path_format(dataset_path), company_group_number, company_number);
	}
	public String get_dataset_company_folderpath(String company_number, String dataset_path)
	{
		int company_group_number = lookup_company_group_number(company_number);
		return get_dataset_company_folderpath(company_number, company_group_number, dataset_path);
	}
}
