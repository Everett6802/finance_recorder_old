...
class WorkdayCalendar
{
	...
	private static WorkdayCalendar instance = null;
	public static WorkdayCalendar get_instance()
	{
		if (instance == null)
			allocate();
		return instance;
	...
	private static synchronized void allocate() // For thread-safe
	{
		if (instance == null)
		{
			instance = new WorkdayCalendar();
			short ret = instance.initialize();
			if (CmnDef.CheckFailure(ret))
			{
				String errmsg = String.format("Fail to initialize the FinanceAnalyzerWorkdayCanlendar object , due to: %s", CmnDef.GetErrorDescription(ret));
				throw new RuntimeException(errmsg);
			}
		} 
	}

	public static class TraverseEntry implements Iterable<CmnClass.FinanceDate>
	{
		private WorkdayCalendar workday_calendar = null;
		private ArrayList<Integer> workday_key_value_sort_array = null;
		private int cur_index = 0;
		private int array_size = 0;

		public TraverseEntry(WorkdayCalendar in_workday_calendar, int start_year, int start_month, int start_day)
		{
			...
			if (!workday_calendar.is_workday(start_year, start_month, start_day))
			{
				...
				CmnLogger.format_error("The day[%s] is NOT a workday, modify it the new one: %s", CmnFunc.transform_date_str(start_year, start_month, start_day), CmnFunc.transform_date_str(next_date_value_list[0], next_date_value_list[1], next_date_value_list[2]));
// Modify the start date if NOT a workday
			...
			start_index = Collections.binarySearch(workday_key_value_sort_array, CmnClass.FinanceDate.get_key_value_from_time_value(start_year, start_month, start_day));
			if (start_index == -1) 
				throw new IllegalArgumentException(String.format("The day[%s] is NOT a workday", CmnFunc.transform_date_str(start_year, start_month, start_day)));
			cur_index = start_index;
			array_size = workday_key_value_sort_array.size();
		}

		@Override
		public Iterator<CmnClass.FinanceDate> iterator()
		{
			Iterator<CmnClass.FinanceDate> it = new Iterator<CmnClass.FinanceDate>()
			{
				private int cur_index = 0;
				@Override
				...
				@Override
				public CmnClass.FinanceDate next()
				{
					int[] time_value_list = CmnClass.FinanceDate.get_time_value_list_from_key_value(workday_key_value_sort_array.get(cur_index));
					cur_index++;
					return new CmnClass.FinanceDate(time_value_list);
				}
				@Override
	...
	public static class ReverseTraverseEntry implements Iterable<CmnClass.FinanceDate>
	{
		private WorkdayCalendar workday_calendar = null;
		private ArrayList<Integer> workday_key_value_sort_array = null;
		private int cur_index = 0;
		public ReverseTraverseEntry(WorkdayCalendar in_workday_calendar, int start_year, int start_month, int start_day)
		{
			workday_calendar = in_workday_calendar;
			workday_key_value_sort_array = workday_calendar.get_workday_key_value_sort_array();
			if (!workday_calendar.is_workday(start_year, start_month, start_day))
			{
				...
				CmnLogger.format_error("The day[%s] is NOT a workday, modify it the new one: %s", CmnFunc.transform_date_str(start_year, start_month, start_day), CmnFunc.transform_date_str(prev_date_value_list[0], prev_date_value_list[1], prev_date_value_list[2]));
// Modify the start date if NOT a workday
				start_year = prev_date_value_list[0];
				start_month = prev_date_value_list[1];
				start_day = prev_date_value_list[2];
			}
			int start_index = Collections.binarySearch(workday_key_value_sort_array, CmnClass.FinanceDate.get_key_value_from_time_value(start_year, start_month, start_day));
			if (start_index == -1) 
				throw new IllegalArgumentException(String.format("The day[%s] is NOT a workday", CmnFunc.transform_date_str(start_year, start_month, start_day)));
			cur_index = start_index;
		}
	...
	private CmnClass.FinanceDateRange finance_date_range = null;
// Caution: the first/last workday is probably not identical to the value in finance date time
	private CmnClass.FinanceDate first_workday_finance_date = null;
	private CmnClass.FinanceDate last_workday_finance_date = null;
...
// Open the file
	...
	private short parse_config()
	{
		...
		short ret = CmnFunc.read_config_file_lines(CmnDef.WORKDAY_CANLENDAR_CONF_FILENAME, config_line_list);
		if (CmnDef.CheckFailure(ret))
...
// Find the time range of the workday calendar in the first line
				...
				if (index == -1)
				{
					CmnLogger.format_error("Incorrect time format in %s: %s", CmnDef.WORKDAY_CANLENDAR_CONF_FILENAME, config_line);
				...
				finance_date_range = new CmnClass.FinanceDateRange(start_date_str, end_date_str);
				CmnLogger.format_debug("Find the time range [%s %s] in %s", start_date_str, end_date_str, CmnDef.WORKDAY_CANLENDAR_CONF_FILENAME);
			}
			else
			{
// Parse the data in each year
				...
				if (year_end_index == -1)
				{
					CmnLogger.format_error("Incorrect data format in %s: %s", CmnDef.WORKDAY_CANLENDAR_CONF_FILENAME, config_line);
					return CmnDef.RET_FAILURE_INCORRECT_CONFIG;
				}
...
// Parse the date in each month
				for (String month_workday_str : workday_month_array_str)
				{
					...
					{
						CmnLogger.format_error("Incorrect month workday list: %s", month_workday_str);
					...
					if (month < 1 || month > 12)
					{
						CmnLogger.format_error("Incorrect month value: %d", month);
						ret = CmnDef.RET_FAILURE_INCORRECT_CONFIG;
						break;
					}
...
//					{
//						CmnLogger.format_error("Incorrect month, expected: %d, actual: %d", month, workday_month_array.size());
//						ret = CmnDef.RET_FAILURE_INCORRECT_CONFIG;
//						break OUT;
//					}
				}
...
// Day
					for (Integer day : day_list)
						workday_key_value_sort_array.add(CmnClass.FinanceDate.get_key_value_from_time_value(year, month, day));
				}
			}
		}
...
//	{
//		CmnClass.FinanceDate finance_date = new CmnClass.FinanceDate(year, month, day);
//		boolean check = check_greater_than_start(finance_date);
//		return check;
//	}
//
//	private boolean check_greater_than_start(final CmnClass.FinanceDate finance_date)
//	{
...
//		return (CmnClass.FinanceDate.get_int_value(finance_date) >= CmnClass.FinanceDate.get_int_value(finance_date_range.get_start_time()));
//	}
	...
	private boolean check_less_than_end(final CmnClass.FinanceDate finance_date)
	{
	...
	private boolean is_workday(int year, int month, int day)
	{
		...
		return CmnDef.CheckSuccess(find_data_pos(year, month, day, date_index_list, TRAVERSE_SEARCH_TYPE.TRAVERSE_SEARCH_EQUAL));
	}

	private boolean is_workday(final CmnClass.FinanceDate finance_date)
	{
	...
	{
		if (!CmnClass.FinanceDateRange.is_in_range(finance_date_range, year, month, day))
		{
			CmnLogger.format_error("The date [%04d-%02d-%02d] is out of range [%s]", year, month, day, finance_date_range.toString());
		...
		{
			CmnLogger.format_error("Incorrect year: %04d", year);
			return CmnDef.RET_FAILURE_INVALID_ARGUMENT;
		}
...
// Find the last day of last month
			if (!found)
			{
				...
				{
					CmnLogger.format_error("The year [%04d] does NOT exist", year);
				...
				{
					CmnLogger.format_error("The day list in [%04d-%02d] should NOT be Empty", year, month);
					return CmnDef.RET_FAILURE_NOT_FOUND;
				}
				date_index_list[0] = year;
				date_index_list[1] = month - 1;
			...
			if (!found)
			{
				CmnLogger.format_error("Fails to find the closest previous workday from [%04d-%02d-%02d]", year, month, day);
				return CmnDef.RET_FAILURE_NOT_FOUND;
			}
		}
...
// Find the first day of next month
			...
			if (!found)
			{
				CmnLogger.format_error("Fails to find the closest next workday from [%04d-%02d-%02d]", year, month, day);
				return CmnDef.RET_FAILURE_NOT_FOUND;
			}
		}
		else
		{
			...
			if (!found)
			{
				CmnLogger.format_warn("The date [%04d-%02d-%02d] is NOT FOUND", year, month, day);
				return CmnDef.RET_FAILURE_NOT_FOUND;
			}
		}
	...
	private short get_date(int year_key, int month_index, int day_index, int[] date_list)
	{
		...
		{
			CmnLogger.format_error("Incorrect year key: %04d", year_key);
			return CmnDef.RET_FAILURE_INVALID_ARGUMENT;
		}
	// Check month index
		if (month_index < 0 || month_index >= 12)
		{
			CmnLogger.format_error("Incorrect month index: %d, should be in the range[0, 12)", month_index);
			return CmnDef.RET_FAILURE_INVALID_ARGUMENT;
		}
		...
		{
			CmnLogger.format_error("Incorrect day index: %d, should be in the range[0, %d)", day_index, day_list_size);
			return CmnDef.RET_FAILURE_INVALID_ARGUMENT;
		}
...
//	private short get_date(int year_key, int month_index, int day_index, CmnClass.FinanceDate finance_date)
//	{
//		int[] date_list = new int[3];
//		short ret = get_date(year_key, month_index, day_index, date_list);
//		if (CmnDef.CheckSuccess(ret))
//		{
//			CmnClass.FinanceDate finance_date_tmp = new CmnClass.FinanceDate(date_list[0], date_list[1], date_list[2]);
//			finance_date = finance_date_tmp;
//		}
//		return ret;
//	}

	public short get_prev_workday_array(int year_base, int month_base, int day_base, ArrayList<CmnClass.FinanceDate> workday_array, int max_workday_amount)
	{
		int[] start_date_index_list = new int[3];
		CmnLogger.format_debug("Try to search for the previous workday list from the date %04d-%02d-%02d......", year_base, month_base, day_base);
...
// Should NOT fail
		if (CmnDef.CheckFailure(ret))
		{
			String errmsg = String.format("Fail to date from the parameter: year_key: %d, month_index: %d, day_index: %d, due to: %s", start_year_key, start_month_index, start_day_index, CmnDef.GetErrorDescription(ret));
			throw new RuntimeException(errmsg);
		}
		int year = date_list[0], month = date_list[1], day = date_list[2];
		CmnLogger.format_info("First workday in the previous workday list: %04d-%02d-%02d......", year, month, day);
...
//				for (int day_index = start_day_index ; day_index >= 0 ; day_index--)
				...
				while (iter.hasPrevious()) 
				{
					...
					CmnClass.FinanceDate finance_date = new CmnClass.FinanceDate(cur_year, cur_month, cur_day);
...
//		for (CmnClass.FinanceDate workday_cfg : workday_array)
//			System.out.printf("%s ", workday_cfg.toString());
//		System.out.printf("\n");
		return CmnDef.RET_SUCCESS;
	}

	public short get_next_workday_array(int year_base, int month_base, int day_base, ArrayList<CmnClass.FinanceDate> workday_array, int max_workday_amount)
	{
		int[] start_date_index_list = new int[3];
		CmnLogger.format_debug("Try to search for the next workday list from the date %04d-%02d-%02d......", year_base, month_base, day_base);
...
// Should NOT fail
		...
		int year = date_list[0], month = date_list[1], day = date_list[2];
		CmnLogger.format_info("First workday in the next workday list: %04d-%02d-%02d......", year, month, day);
...
//		System.out.printf("\n");
	...
	{
// Find the date
		ArrayList<CmnClass.FinanceDate> workday_array = new ArrayList<CmnClass.FinanceDate>();
		short ret = get_prev_workday_array(year_base, month_base, day_base, workday_array, 1);
		if (CmnDef.CheckFailure(ret))
			return ret;
		if (workday_array.isEmpty())
		{
			CmnLogger.warn("No data is FOUND");
			return CmnDef.RET_FAILURE_NOT_FOUND;
		}
// Update the data
		CmnClass.FinanceDate finance_date = workday_array.get(0);
		finance_date.update_time_value_list(prev_date_value_list);

		return CmnDef.RET_SUCCESS;
	}

//	public short get_prev_workday(final CmnClass.FinanceDate finance_date, CmnClass.FinanceDate prev_finance_date)
//	{
//		int[] prev_date_value_list = new int[3];
...
//			prev_finance_date = new CmnClass.FinanceDate(prev_year, prev_month, prev_day);
//		}
//		return ret;
//	}
...
//	public short get_next_workday(final CmnClass.FinanceDate finance_date, CmnClass.FinanceDate next_finance_date)
//	{
//		int[] next_date_value_list = new int[3];
...
//			next_finance_date = new CmnClass.FinanceDate(prev_year, prev_month, prev_day);
//		}
//		return ret;
//	}
		...
		if (first_workday_finance_date != null)
		{
			...
			for (int month_index = 0 ; month_index < 12 ; month_index++)
			{
				...
				if (!day_list.isEmpty())
				{
					...
					month = month_index + 1;
					day = day_list.get(0);
					first_workday_finance_date = new CmnClass.FinanceDate(year, month, day);
					break;
				}
			}
			assert last_workday_finance_date != null : "first_workday_finance_date should NOT be NULL";
			CmnLogger.format_debug("The first workday: %s", first_workday_finance_date.toString());
		}
...
//	public short get_first_workday(CmnClass.FinanceDate first_finance_date)
//	{
//		int[] first_date_list = new int[3];
//		short ret = get_first_workday(first_date_list);
//		if (CmnDef.CheckSuccess(ret))
//			first_finance_date = new CmnClass.FinanceDate(first_date_list[0], first_date_list[1], first_date_list[2]);
//		return ret;
//	}
		...
		if (last_workday_finance_date != null)
		{
			...
			for (int month_index = 11 ; month_index >= 0 ; month_index--)
			{
				...
				if (!day_list.isEmpty())
				{
					...
					last_workday_finance_date = new CmnClass.FinanceDate(year, month, day);
					break;
				}
			}
			assert last_workday_finance_date != null : "last_workday_finance_date should NOT be NULL";
			CmnLogger.format_debug("The last workday: %s", last_workday_finance_date.toString());
		}
...
//	public short get_last_workday(CmnClass.FinanceDate last_finance_date)
//	{
//		int[] last_date_list = new int[3];
//		short ret = get_last_workday(last_date_list);
//		if (CmnDef.CheckSuccess(ret))
//			last_finance_date = new CmnClass.FinanceDate(last_date_list);
//		return ret;
//	}

//	static int main(String args[])
//	{
//		WorkdayCalendar workday_calendar = get_instance();
//		CmnClass.FinanceDate finance_date = null;
//
//		finance_date = new CmnClass.FinanceDate(2016, 6, 1);
