...
class MarketDataHandler extends DataHandlerBase
{
	...
	{
		return String.format("%s/%s/%s.csv", csv_folderpath, CmnDef.CSV_MARKET_FOLDERNAME,
	...
	private static String get_csv_urlpath(String csv_server_ip, int method_index)
	{
		...
		return String.format("%s/%s/%s.csv", csv_url_folderpath, CmnDef.CSV_MARKET_FOLDERNAME,
				CmnDef.FINANCE_DATA_NAME_LIST[method_index]);
	}
	...
	// private LinkedList<CmnClass.MethodTimeRange> method_time_range_list =
	// null;
	// private String current_csv_working_folerpath =
	// CmnDef.BACKUP_CSV_ROOT_FOLDERPATH;
	...
	protected short create_finance_folder_hierarchy(String root_folderpath)
	{
		...
		if (CmnDef.CheckFailure(ret))
			CmnLogger.format_error("Fail to create the root folder[%s], due to: %s", root_folderpath,
	...
	protected short parse_missing_csv()
	{
		...
		short ret = CmnFunc.read_config_file_lines(CmnDef.MISSING_CSV_MARKET_FILENAME, current_csv_working_folerpath,
				config_line_list);
		if (CmnDef.CheckFailure(ret))
		{
	...
	public short read_from_csv(CSVHandlerMap csv_data_map)
	{
		...
		for (Integer method_index : method_index_list)
		{
			CSVHandler csv_reader = CSVHandler
					.get_csv_reader(MarketDataHandler.get_csv_filepath(current_csv_working_folerpath, method_index));
			if (csv_reader == null)
			{
	...
	public short write_into_sql(final CSVHandlerMap csv_data_map)
	{
		...
		short ret = CmnDef.RET_SUCCESS;
		// Establish the connection to the MySQL and create the database if not
		// exist
		...
		OUT: for (Integer method_index : method_index_list)
		{
			// Check data exist
	...
	@Override
	public short read_from_sql(CmnClass.QuerySet query_set, CmnClass.FinanceTimeRange finance_time_range,
			CmnClass.ResultSetMap result_set_map)
	{
		assert method_index_list != null : "method_index_list == NULL";
		
		// CAUTION: The data set in the reslt_set variable should be added
		// before calling this function
		// Set the mapping table of reading the specific CSV files and writing
		...
		OUT: switch (data_unit)
		{
			case ResultSetDataUnit_NoMethod:
			{
				result_set = new CmnClass.ResultSet();
				// Add query set
				// for (CmnClass.MethodTimeRange method_time_range :
				// method_time_range_list)
	...
	public short write_into_csv(CmnClass.ResultSetMap result_set_map)
	{
		...
		OUT: switch (data_unit)
		{
			case ResultSetDataUnit_NoMethod:
			{
				...
				{
					CmnLogger.format_error("No data found in the source key: %d",
							CmnDef.NO_SOURCE_TYPE_MARKET_SOURCE_KEY_VALUE);
					ret = CmnDef.RET_FAILURE_INVALID_ARGUMENT;
					break OUT;
				}
				for (Integer method_index : method_index_list)
				{
					CSVHandler csv_writer = CSVHandler.get_csv_writer(
							MarketDataHandler.get_csv_filepath(current_csv_working_folerpath, method_index));
	...
	public short transfrom_sql_to_csv(CmnClass.QuerySet query_set, CmnClass.FinanceTimeRange finance_time_range)
	{
		...
		OUT: for (Integer method_index : method_index_list)
		{
			...
			CSVHandler csv_writer = CSVHandler
					.get_csv_writer(MarketDataHandler.get_csv_filepath(current_csv_working_folerpath, method_index));
			// Assemble the data and write into CSV
			ArrayList<String> csv_data_list = result_set.to_string_array(method_index);
			csv_writer.set_write_data(csv_data_list);
			ret = csv_writer.write();
	...
	public short cleanup_sql()
	{
		...
		if (CmnDef.CheckFailure(ret))
		{
			if (operation_can_continue(ret))
			{
				...
				String errmsg = String.format("Fails to delete database[%s], due to: %s",
						CmnDef.SQL_MARKET_DATABASE_NAME, CmnDef.GetErrorDescription(ret));
	...
	public short check_sql_exist(ArrayList<String> not_exist_list)
	{
		...
		if (CmnDef.CheckFailure(ret))
		{
			...
			{
				CmnLogger.format_warn("The database[%s] does NOT exist, add all tables in the Not-Found List......",
						CmnDef.SQL_MARKET_DATABASE_NAME);
