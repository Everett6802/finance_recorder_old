...
class CmnClass 
{
	public static abstract class FinanceTimeBase
	{
		...
		public abstract CmnDef.FinanceTimeUnit get_time_unit();
	};

	public static class FinanceDate extends FinanceTimeBase implements Comparable<Object>
	{
		...
		{
// Check Year Range
			CmnFunc.check_year_range(year);
// Check Month Range
			CmnFunc.check_month_range(month);
// Check Month Range
			CmnFunc.check_day_range(day, year, month);
		}
...
// For MySQL
		...
		{
			return get_java_date_object(CmnFunc.transform_date_str(year, month, day));
		}
		...
		{
			int[] value = CmnFunc.get_date_value_list_from_str(finance_date_str);
			year = value[0];
			month = value[1];
			day = value[2];
		}
		...
		public CmnDef.FinanceTimeUnit get_time_unit(){return CmnDef.FinanceTimeUnit.FinanceTime_Date;}
		...
		public String toString()
		{
			if (time_description == null)
				time_description = CmnFunc.transform_date_str(year, month, day);
		...
		public FinanceQuarter get_quarter_object(){return new FinanceQuarter(year, CmnFunc.get_quarter_from_month(month));}
	};

	public static class FinanceMonth extends FinanceTimeBase implements Comparable<Object>
	{
...
// Check Month Range
		...
		{
			int[] value = CmnFunc.get_month_value_list_from_str(finance_month_str);
			year = value[0];
			month = value[1];
		}
		...
		public CmnDef.FinanceTimeUnit get_time_unit(){return CmnDef.FinanceTimeUnit.FinanceTime_Month;}

		@Override
		public String toString()
		{
			if (time_description == null)
				time_description = CmnFunc.transform_month_str(year, month);
			return time_description;
		}
		...
		public FinanceDate get_date_end_object(){return new FinanceDate(year, month, CmnFunc.get_month_last_day(year, month));}
		public FinanceQuarter get_quarter_object(){return new FinanceQuarter(year, CmnFunc.get_quarter_from_month(month));}
	};

	public static class FinanceQuarter extends FinanceTimeBase implements Comparable<Object>
	{
...
// Check Year Range
			CmnFunc.check_year_range(year);
// Check Quarter Range
			CmnFunc.check_quarter_range(quarter);
		}
		public static int[] get_time_value_list_from_key_value(int key_value)
		{
		...
		{
			int[] value = CmnFunc.get_quarter_value_list_from_str(finance_quarter_str);
			year = value[0];
			quarter = value[1];
		}
		...
		public CmnDef.FinanceTimeUnit get_time_unit(){return CmnDef.FinanceTimeUnit.FinanceTime_Quarter;}

		@Override
		public String toString()
		{
			if (time_description == null)
				time_description = CmnFunc.transform_quarter_str(year, quarter);
			return time_description;
		}
		...
		public FinanceDate get_date_start_object(){return new FinanceDate(year, CmnFunc.get_quarter_first_month(quarter), 1);}
		public FinanceDate get_date_end_object()
		{
			int month = CmnFunc.get_quarter_last_month(quarter);
			return new FinanceDate(year, month, CmnFunc.get_quarter_last_day(month));
		}
		public FinanceMonth get_month_start_object(){return new FinanceMonth(year, CmnFunc.get_quarter_first_month(quarter));}
		public FinanceMonth get_month_end_object(){return new FinanceMonth(year, CmnFunc.get_quarter_last_month(quarter));}
	};

	public static class FinanceTime
	{
		private FinanceTimeBase finance_time = null;
		private CmnDef.FinanceTimeUnit finance_time_unit = CmnDef.FinanceTimeUnit.FinanceTime_Undefined;

		public FinanceTime(FinanceDate in_finance_date)
		{
			finance_time = new FinanceDate(in_finance_date);
			finance_time_unit = CmnDef.FinanceTimeUnit.FinanceTime_Date;
		}
		public FinanceTime(FinanceMonth in_finance_month)
		{
			finance_time = new FinanceMonth(in_finance_month);
			finance_time_unit = CmnDef.FinanceTimeUnit.FinanceTime_Month;
		}
		public FinanceTime(FinanceQuarter in_finance_quarter)
		{
			finance_time = new FinanceQuarter(in_finance_quarter);
			finance_time_unit = CmnDef.FinanceTimeUnit.FinanceTime_Quarter;
		}
		public FinanceTime(String in_finance_time)
		{
			finance_time_unit = CmnFunc.get_time_unit_from_string(in_finance_time);
// Initialize the instance
		...
		public FinanceTime(int[] time_value_list, CmnDef.FinanceTimeUnit in_finance_time_unit)
		{
// Initialize the instance
		...
		public FinanceTime(int key_value, CmnDef.FinanceTimeUnit in_finance_time_unit)
		{
// Initialize the instance
			switch (in_finance_time_unit)
			{
		...
		public CmnDef.FinanceTimeUnit get_time_unit(){return finance_time_unit;}
	};

	public abstract static class FinanceTimeRangeBase
	{
		protected static CmnDef.FinanceTimeRangeType get_time_range_type(Object time_start, Object time_end)
		{
			if (time_start == null && time_end == null)
				throw new IllegalArgumentException("start and end time can NOT be null at the same time");
			if (time_start == null)
				return CmnDef.FinanceTimeRangeType.FinanceTimeRange_LessEqual;
			else if (time_end == null)
				return CmnDef.FinanceTimeRangeType.FinanceTimeRange_GreaterEqual;
			return CmnDef.FinanceTimeRangeType.FinanceTimeRange_Between;
		}

		protected CmnDef.FinanceTimeRangeType finance_time_range_type = null;

		public CmnDef.FinanceTimeRangeType get_time_range_type(){return finance_time_range_type;}
...
//			return false;
		...
		{
			finance_time_range_type = CmnDef.FinanceTimeRangeType.FinanceTimeRange_Between;
	...
	public static class FinanceTimeRange// extends FinanceTimeRangeBase
	{
		...
		private CmnDef.FinanceTimeUnit finance_time_unit = CmnDef.FinanceTimeUnit.FinanceTime_Undefined;
		private CmnDef.FinanceTimeRangeType finance_time_range_type = CmnDef.FinanceTimeRangeType.FinanceTimeRange_Undefined;
		...
		public FinanceTimeRange(String in_finance_time_start, String in_finance_time_end)
		{
			...
			case FinanceTimeRange_Between:
			{
				CmnDef.FinanceTimeUnit finance_start_time_unit = CmnFunc.get_time_unit_from_string(in_finance_time_start);
				CmnDef.FinanceTimeUnit finance_end_time_unit = CmnFunc.get_time_unit_from_string(in_finance_time_end);
			...
			case FinanceTimeRange_LessEqual:
			{
				finance_time_unit = CmnFunc.get_time_unit_from_string(in_finance_time_end);
			}
			break;
			case FinanceTimeRange_GreaterEqual:
			{
				finance_time_unit = CmnFunc.get_time_unit_from_string(in_finance_time_start);
			}
			break;
			default:
...
// Initialize the instance
	...
	{
		public static LinkedList<SourceTypeTimeRange> get_whole_method_time_range_list()
		{
			LinkedList<SourceTypeTimeRange> method_time_range_list = new LinkedList<SourceTypeTimeRange>();
			for(int method_index : CmnFunc.get_all_method_index_list())
			{
				method_time_range_list.add(new SourceTypeTimeRange(method_index));
			}
			return method_time_range_list;
		}
		private int method_index;
		private FinanceTimeRange finance_time_range = null;

		public SourceTypeTimeRange(int in_method_index, FinanceTimeRange in_finance_time_range)
		{
			method_index = in_method_index;
			finance_time_range = new FinanceTimeRange(in_finance_time_range);
		}

		public SourceTypeTimeRange(int in_method_index, FinanceDate finance_date_start, FinanceDate finance_date_end)
		{
			this(in_method_index, new FinanceTimeRange(finance_date_start, finance_date_end));
//			this.method_index = in_method_index;
//			this.finance_time_range = new FinanceTimeRange(finance_date_start, finance_date_end);
		}
		public SourceTypeTimeRange(int in_method_index, FinanceMonth finance_month_start, FinanceMonth finance_month_end)
		{
			this(in_method_index, new FinanceTimeRange(finance_month_start, finance_month_end));
//			this.method_index = in_method_index;
//			this.finance_time_range = new FinanceTimeRange(finance_month_start, finance_month_end);
		}
		public SourceTypeTimeRange(int in_method_index, FinanceQuarter finance_quarter_start, FinanceQuarter finance_quarter_end)
		{
			this(in_method_index, new FinanceTimeRange(finance_quarter_start, finance_quarter_end));
//			this.method_index = in_method_index;
//			this.finance_time_range = new FinanceTimeRange(finance_quarter_start, finance_quarter_end);
		}
		public SourceTypeTimeRange(int in_method_index, String in_finance_time_start, String in_finance_time_end)
		{
			this.method_index = in_method_index;
		...
		{
			this.method_index = another.method_index;
			this.finance_time_range = new FinanceTimeRange(another.finance_time_range);
		}

		public SourceTypeTimeRange(int in_method_index)
		{
//			super(in_method_index, new FinanceTimeRange(finance_quarter_start, finance_quarter_end));
			this.method_index = in_method_index;
			this.finance_time_range = new FinanceTimeRange(CmnDef.DEF_START_DATE_STR, CmnDef.DEF_END_DATE_STR);
		}

		@Override
		public int compareTo(SourceTypeTimeRange other) 
		{
			return (method_index - other.method_index);
		}

		public int get_method_index(){return method_index;}
		public FinanceTimeRange get_time_range(){return finance_time_range;}
		public CmnDef.FinanceTimeRangeType get_time_range_type(){return finance_time_range.get_time_range_type();}
		public CmnDef.FinanceTimeUnit get_time_unit(){return finance_time_range.get_time_unit();}
...
//			if (!matcher.find())
//			{
////				CmnDef.format_error("Incorrect time format: %s", time_str);
//				return null;
//			}
//			return matcher;
//		}
...
//		public final T[] get_data_array(){return array_data;}
		...
		public final T get_index(int index)
		{
			...
			if (index < 0)
				index = CmnFunc.get_index_ex(index, array_size);
	...
	public static class QuerySet
	{
		...
		private LinkedList<Integer> query_method_index_list = null;
...
//				query_array.add(new LinkedList<Integer>());
		...
		public short add_query(int method_index, int field_index)
		{
			if (add_done)
			{
				CmnLogger.error("Fail to add another data");
...
// Check if the index is out of range
			...
			{
				CmnLogger.format_error("method_index[%d] is out of range in QuerySet", method_index);
				return CmnDef.RET_FAILURE_INVALID_ARGUMENT;
			}

			if(field_index < 0 || field_index >= CmnDef.FINANCE_DATABASE_FIELD_AMOUNT_LIST[method_index])
			{
// If field_index == -1, it means select all field in the table
				if (field_index != -1)
				{
					CmnLogger.error("field_index is out of range in QuerySet");
					return CmnDef.RET_FAILURE_INVALID_ARGUMENT;
				}
			}
			if (!query_map.containsKey(method_index))
				query_map.put(method_index, new LinkedList<Integer>());
			else
			{
// Check the index is duplicate
				if (query_map.get(method_index).indexOf(field_index) != -1)
				{
					CmnLogger.format_warn("Duplicate index: %d in %s", field_index, CmnDef.FINANCE_METHOD_DESCRIPTION_LIST[method_index]);
					return CmnDef.RET_FAILURE_WARN_INDEX_DUPLICATE;
				}
// If all fields are selected, it's no need to add extra index
				if (!query_map.get(method_index).isEmpty() && query_map.get(method_index).get(0) == -1)
				{
					CmnLogger.format_warn("Ignore index: %d in %s", field_index, CmnDef.FINANCE_METHOD_DESCRIPTION_LIST[method_index]);
					return CmnDef.RET_FAILURE_WARN_INDEX_IGNORE;
				}
// Clear the old index if all data are selected
				if (field_index == -1)
					query_map.get(method_index).clear();
			}
// Add the index
			query_map.get(method_index).add(field_index);
			return CmnDef.RET_SUCCESS;
		}
		public short add_query(int method_index){return add_query(method_index, -1);}
...
//			for (int i = 0 ; i < CmnDef.FINANCE_SOURCE_SIZE ; i++)
			{
				...
				if (!CmnFunc.check_method_index_in_range(method_index))
					throw new IllegalStateException(String.format("Unsupported source type index: %d", method_index));
...
//				WRITE_FORMAT_DEBUG("Transform the query data[method_index: %d]", i);
				...
				{
					field_list.clear();
					for (int field_index = 1 ; field_index < CmnDef.FINANCE_DATABASE_FIELD_AMOUNT_LIST[method_index] ; field_index++) // Caution: Don't include the "date" field
		...
		public final LinkedList<Integer> get_method_index_list()
		{
			if (query_method_index_list == null)
			{
				query_method_index_list = new LinkedList<Integer>();
				for(Map.Entry<Integer, LinkedList<Integer>> entry : query_map.entrySet())
				{
					int method_index = entry.getKey();
					if (CmnFunc.check_method_index_in_range(method_index))
						throw new IllegalStateException(String.format("Unsupported source type index: %d", method_index));
					query_method_index_list.add(method_index);
				}
			}
			return query_method_index_list;
		}

		public final LinkedList<Integer> get_field_index_list(int method_index)
		{
			int[] index_array = CmnFunc.get_method_index_range();
			if (method_index < index_array[0] || method_index >= index_array[1])
			{
				String errmsg = String.format("The index[%d] is out of range(0, %d)", method_index , index_array[1] - 1);
				CmnLogger.error(errmsg);
				throw new IndexOutOfBoundsException(errmsg);
			}
			return (query_map.containsKey(method_index) ? query_map.get(method_index) : null);
		}
	};

	static public short add_query(QuerySet query_set, CmnDef.FinanceMethod method, int field_index){return query_set.add_query(method.value(), field_index);}
	static public short add_query_ex(QuerySet query_set, CmnDef.FinanceMethod method, int field_index, HashSet<Integer> method_index_set)
	{
		method_index_set.add(method.value());
		return add_query(query_set, method, field_index);
	}
...
//			if (is_add_done)
//			{
//				CmnDef.format_error("The add_done flag has already been set to True");
//				return CmnDef.RET_FAILURE_INCORRECT_OPERATION;
//			}
...
//				{
//					CmnDef.format_error("The company group[%d] has already been set to NULL", company_group_number);
//					return CmnDef.RET_FAILURE_INCORRECT_OPERATION;
//				}
//			}
...
//				{
//					CmnDef.format_warn("The company code number[%s] has already been added to the group[%d]", company_code_number, company_group_number);
//					continue;
//				}
...
//			{
//				if (company_number_in_group_map.get(company_group_number) != null)
//					CmnDef.format_warn("Select all company group[%d], ignore the original settings......", company_group_number);
//			}
...
//	};
	...
	public static class ResultSet
	{
		...
		private LinkedList<Integer> method_index_list = null;
...
//		}
		...
		public short add_set(int method_index, int field_index)
		{
			if(!CmnFunc.check_method_index_in_range(method_index))
			{
				CmnLogger.error("method_index is out of range in ResultSet");
				return CmnDef.RET_FAILURE_INVALID_ARGUMENT;
			}
			if(field_index < 0 || field_index >= CmnDef.FINANCE_DATABASE_FIELD_AMOUNT_LIST[method_index])
			{
...
//				if (field_index != -1)
//				{
					CmnLogger.error("field_index is out of range in ResultSet");
					return CmnDef.RET_FAILURE_INVALID_ARGUMENT;
//				}
			}
...
//			}
			...
			{
// Check if the method:field_type has been set
				short key = get_combined_index(method_index, index);
				if (data_set_mapping.get(key) != null)
				{
					CmnLogger.format_error("The key[%d] from (%d, %d) is duplicate", key, method_index, index);
					return CmnDef.RET_FAILURE_INVALID_ARGUMENT;
				}
// Check the lookup table to find the date type of the specific field
				short value;
				switch(CmnDef.FINANCE_DATABASE_FIELD_TYPE_LIST[method_index][field_index])
				{
				case FinanceField_INT:
					value = get_combined_index(CmnDef.FinanceFieldType.FinanceField_INT.value(), int_data_set.size());
					int_data_set.add(new FinanceIntDataArray());
					int_data_set_size = int_data_set.size();
					break;
				case FinanceField_LONG:
					value = get_combined_index(CmnDef.FinanceFieldType.FinanceField_LONG.value(), long_data_set.size());
					long_data_set.add(new FinanceLongDataArray());
					long_data_set_size = long_data_set.size();
					break;
				case FinanceField_FLOAT:
					value = get_combined_index(CmnDef.FinanceFieldType.FinanceField_FLOAT.value(), float_data_set.size());
					float_data_set.add(new FinanceFloatDataArray());
					float_data_set_size = float_data_set.size();
					break;
				case FinanceField_DATE:
					CmnLogger.error("The DATE field type is NOT supported");
					return CmnDef.RET_FAILURE_INVALID_ARGUMENT;
				default:
					CmnLogger.format_error("The unsupported field type: %d", CmnDef.FINANCE_DATABASE_FIELD_TYPE_LIST[method_index][field_index]);
					return CmnDef.RET_FAILURE_INVALID_ARGUMENT;
				}
				CmnLogger.format_debug("ResultSet Map: %d, %d", key, value);
...
// Keep track of the source type index
		...
		public short add_set(int method_index, final LinkedList<Integer> field_list)
		{
			...
			while (iter.hasNext())
			{
				...
				ret = add_set(method_index, field_index);
		...
		public short set_date(String element_value)
		{
			if (check_date_data_mode)
			{
				...
				{
					CmnLogger.format_error("The date(%s, %s) is NOT equal", date_data.get_index(date_data_cur_pos), element_value);
					return CmnDef.RET_FAILURE_INCORRECT_OPERATION;
				}
				date_data_cur_pos++;
			}
...
//				throw new IndexOutOfBoundsException(String.format("The Index [%d] is out of range [0, %d)", index, date_data_size));
		...
		public String[] get_date_array_elements(int method_index, int field_index, int start_index, int end_index) 
		{
			if (start_index < 0)
				start_index = CmnFunc.get_start_index_ex(start_index, date_data_size);
			if (end_index < 0)
				end_index = CmnFunc.get_end_index_ex(end_index, date_data_size);
			if (!CmnFunc.check_start_index_in_range(start_index, 0, date_data_size))
				throw new IndexOutOfBoundsException(String.format("The Start Index [%d] is NOT in range [0, %d)", start_index, date_data_size));
			if (!CmnFunc.check_end_index_in_range(end_index, 1, date_data_size))
		...
		private short find_data_pos(int method_index, int field_index, short[] field_type_array)
		{
			short key = get_combined_index(method_index, field_index);
			if (data_set_mapping.get(Integer.valueOf(key)) == null)
			{
				CmnLogger.format_error("The key[%d] from (%d, %d) is NOT FOUND", key, method_index, field_index);
				return CmnDef.RET_FAILURE_INVALID_ARGUMENT;
			}
		...
		private void find_and_check_data_pos(int method_index, int field_index, short[] field_type_array)
		{
			if (CmnDef.CheckFailure(find_data_pos(method_index, field_index, field_type_array)))
				throw new IllegalArgumentException(String.format("The key[%d] from (%d, %d) is NOT FOUND", method_index, field_index));
		}

		public short set_data(int method_index, int field_index, Object data_obj)
		{
			short[] field_type_array = new short[2];
			short ret = find_data_pos(method_index, field_index, field_type_array);
			if (CmnDef.CheckFailure(ret))
				return ret;

			short field_type_index = field_type_array[0];
			short field_type_pos = field_type_array[1];
			switch(CmnDef.FinanceFieldType.valueOf(field_type_index))
			{
			case FinanceField_INT:
			...
			case FinanceField_FLOAT:
				...
				break;
			default:
				CmnLogger.format_error("Unsupported field_type_index: %d", field_type_index);
				return CmnDef.RET_FAILURE_INVALID_ARGUMENT;
			}
			return CmnDef.RET_SUCCESS;
		}
...
// Check all the data size are equal
				if (date_data_size != data_size)
				{
					...
					short method_index = get_upper_subindex(key);
					short field_index = get_lower_subindex(key);
					CmnLogger.format_error("Incorrect data size in %d, %d, expected: %d, actual: %d", method_index, field_index, date_data_size, data_size);
					return CmnDef.RET_FAILURE_INCORRECT_OPERATION;
				}
			}
			return CmnDef.RET_SUCCESS;
		...
		// Show the database:field info
			...
			for(int i = 0 ; i < STAR_LEN ; i++)
				star_str += "*";
			System.out.printf("%s\n| %s |", star_str, CmnDef.MYSQL_DATE_FILED_NAME);
			...
			for(Map.Entry<Integer, Integer> entry : data_set_mapping.entrySet())
			{
				...
				System.out.printf(" %s:%s%d |", CmnDef.FINANCE_METHOD_DESCRIPTION_LIST[method_index], CmnDef.MYSQL_FILED_NAME_BASE, field_index);
			}
...
// Show the data info
		...
		public final FinanceIntDataArray get_int_array(int method_index, int field_index)
		{
			short[] field_type_array = new short[2];
			find_and_check_data_pos(method_index, field_index, field_type_array);
			check_data_pos_boundary(field_type_array[0], field_type_array[1], CmnDef.FinanceFieldType.FinanceField_INT.value(), int_data_set_size);
			assert field_type_array[0] == CmnDef.FinanceFieldType.FinanceField_INT.value() : String.format("The type[%d] is NOT INT", CmnDef.FinanceFieldType.FinanceField_FLOAT.value());
			return int_data_set.get(field_type_array[1]);
		}
		public final FinanceLongDataArray get_long_array(int method_index, int field_index)
		{
			short[] field_type_array = new short[2];
			find_and_check_data_pos(method_index, field_index, field_type_array);
			check_data_pos_boundary(field_type_array[0], field_type_array[1], CmnDef.FinanceFieldType.FinanceField_LONG.value(), long_data_set_size);
			assert field_type_array[0] == CmnDef.FinanceFieldType.FinanceField_LONG.value() : String.format("The type[%d] is NOT LONG", CmnDef.FinanceFieldType.FinanceField_FLOAT.value());
			return long_data_set.get(field_type_array[1]);
		}
		public final FinanceFloatDataArray get_float_array(int method_index, int field_index)
		{
			short[] field_type_array = new short[2];
			find_and_check_data_pos(method_index, field_index, field_type_array);
			check_data_pos_boundary(field_type_array[0], field_type_array[1], CmnDef.FinanceFieldType.FinanceField_FLOAT.value(), float_data_set_size);
			assert field_type_array[0] == CmnDef.FinanceFieldType.FinanceField_FLOAT.value() : String.format("The type[%d] is NOT FLOAT", CmnDef.FinanceFieldType.FinanceField_FLOAT.value());
			return float_data_set.get(field_type_array[1]);
		}
		public int get_int_array_element(int method_index, int field_index, int index) 
		{
			if (index < 0 || index >= date_data_size)
				throw new IndexOutOfBoundsException(String.format("The Index [%d] is out of range [0, %d)", index, date_data_size));
			return get_int_array(method_index, field_index).get_index(index);
		}
		public long get_long_array_element(int method_index, int field_index, int index) 
		{
			if (index < 0 || index >= date_data_size)
				throw new IndexOutOfBoundsException(String.format("The Index [%d] is out of range [0, %d)", index, date_data_size));
			return get_long_array(method_index, field_index).get_index(index);
		}
		public float get_float_array_element(int method_index, int field_index, int index) {return get_float_array(method_index, field_index).get_index(index);}

		public int[] get_int_array_elements(int method_index, int field_index, int start_index, int end_index) 
		{
			...
			FinanceIntDataArray int_data_array = get_int_array(method_index, field_index);
			int data_pos = 0;
			for (int index = start_index ; index < end_index ; index++)
				data_array[data_pos++] = int_data_array.get_index(index);
			return data_array;
		}
		public long[] get_long_array_elements(int method_index, int field_index, int start_index, int end_index) 
		{
			if (start_index < 0 || start_index >= date_data_size)
				throw new IndexOutOfBoundsException(String.format("The Start Index [%d] is out of range [0, %d)", start_index, date_data_size));
			if (end_index < 1 || end_index > date_data_size)
			...
			FinanceLongDataArray long_data_array = get_long_array(method_index, field_index);
			int data_pos = 0;
			for (int index = start_index ; index < end_index ; index++)
				data_array[data_pos++] = long_data_array.get_index(index);
			return data_array;
		}
		public float[] get_float_array_elements(int method_index, int field_index, int start_index, int end_index) 
		{
			if (start_index < 0 || start_index >= date_data_size)
				throw new IndexOutOfBoundsException(String.format("The Start Index [%d] is out of range [0, %d)", start_index, date_data_size));
			if (end_index < 1 || end_index > date_data_size)
			...
			FinanceFloatDataArray float_data_array = get_float_array(method_index, field_index);
			int data_pos = 0;
			for (int index = start_index ; index < end_index ; index++)
		...
		public ArrayList<String> to_string_array(int method_index, int start_index, int end_index)
		{
			if (start_index < 0 || start_index >= date_data_size)
				throw new IndexOutOfBoundsException(String.format("The Start Index [%d] is out of range [0, %d)", start_index, date_data_size));
			if (end_index < 1 || end_index > date_data_size)
...
// Keep track of the data string into array
			...
			for (int field_index = 1 ; field_index < CmnDef.FINANCE_DATABASE_FIELD_AMOUNT_LIST[method_index] ; field_index++)
			{
				switch(CmnDef.FINANCE_DATABASE_FIELD_TYPE_LIST[method_index][field_index])
				{
				case FinanceField_INT:
				{
					int[] data_array = get_int_array_elements(method_index, field_index, start_index, end_index);
					data_pos = 0;
				...
				case FinanceField_LONG:
				{
					long[] data_array = get_long_array_elements(method_index, field_index, start_index, end_index);
					data_pos = 0;
					for (int index = start_index ; index < end_index ; index++, data_pos++)
						data_string_array[data_pos] += String.format(",%d", data_array[data_pos]);
				}
				...
				case FinanceField_FLOAT:
				{
					float[] data_array = get_float_array_elements(method_index, field_index, start_index, end_index);
					data_pos = 0;
					for (int index = start_index ; index < end_index ; index++, data_pos++)
				...
				default:
				{
					String errmsg = String.format("The unsupported field type: %d", CmnDef.FINANCE_DATABASE_FIELD_TYPE_LIST[method_index][field_index]);
		...
		public ArrayList<String> to_string_array(int method_index)
		{
			return to_string_array(method_index, 0, date_data_size);
		}

//		public String[] get_array_all_elements_string_list(int method_index, int start_index, int end_index)
//		{
//			if (start_index < 0 || start_index >= date_data_size)
//				throw new IndexOutOfBoundsException(String.format("The Start Index [%d] is out of range [0, %d)", start_index, date_data_size));
//			if (end_index < 1 || end_index > date_data_size)
...
//		}
	...
	public static class ResultSetMap implements Iterable<Map.Entry<Integer, ResultSet>>
	{
		...
		private CmnDef.ResultSetDataUnit result_set_data_unit = CmnDef.ResultSetDataUnit.ResultSetDataUnit_NoMethod;

		public ResultSetMap(){}
		public ResultSetMap(CmnDef.ResultSetDataUnit data_unit){result_set_data_unit = data_unit;}

		public CmnDef.ResultSetDataUnit get_data_unit(){return result_set_data_unit;}
