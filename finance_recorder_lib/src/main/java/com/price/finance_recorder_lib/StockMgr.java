...
public class StockMgr extends MgrBase
{
	...
	// if (company_group_set == null)
	// company_group_set =
	// CmnClassStock.CompanyGroupSet.get_whole_company_group_set();
	// }
	// return ret;
	// }
	
	@Override
	public void set_delete_sql_accuracy(CmnDef.DeleteSQLAccurancyType accurancy_type)
	{
		delete_sql_accurancy_type = accurancy_type;
	}
	
	private short transform_company_word_list_to_group_set(List<String> company_word_list)
	{
		/*
		 * The argument type: Company code number: 2347 Company code number
		 * range: 2100-2200 Company group number: [Gg]12 Company code
		 * number/number range hybrid: 2347,2100-2200,2362,g2,1500-1510
		 */
				...
				if (matcher1 == null)
				{
					...
					if (CmnFunc.get_regex_matcher("([\\d]{4})", company_number) == null)
						throw new IllegalStateException(
								String.format("Unknown company number format: %s", company_number));
	...
	public short transfrom_csv_to_sql_multithread(int sub_company_group_set_amount, boolean stop_when_csv_not_foud)
	{
		...
		ArrayList<CmnClassStock.CompanyGroupSet> sub_company_group_set_list = company_group_set
				.get_sub_company_group_set_list(sub_company_group_set_amount);
		// Create thread pool
		...
		for (CmnClassStock.CompanyGroupSet sub_company_group_set : sub_company_group_set_list)
		{
			...
			StockWriteSQLTask task = new StockWriteSQLTask(method_index_list, sub_company_group_set,
					current_csv_working_folerpath, stop_when_csv_not_foud);
		...
		for (Future<Integer> res : res_list)
		{
			...
			if (CmnDef.CheckFailure(ret))
			{
				CmnLogger.format_error("Fail to get return value in thread[%d], due to: %s", res_index,
