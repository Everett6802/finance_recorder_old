...
//		String conf_filepath = String.format("%s/%s/%s", CmnDef.get_current_path(), CmnDef.CONF_FOLDERNAME, CmnDef.COMPANY_PROFILE_CONF_FILENAME);
//		CmnLogger.format_debug("Try to parse the configuration in %s", conf_filepath);
// Check the file exists or not
//		File fp = new File(conf_filepath);
//		if (!fp.exists())
//		{
//			CmnLogger.format_error("The configration file[%s] does NOT exist", conf_filepath);
//			return CmnDef.RET_FAILURE_NOT_FOUND;
//		}
//		CmnLogger.format_debug("Try to parse the config: %s", conf_filepath);
...
//				String data_array[] = buf.split(CmnDef.COMMA_DATA_SPLIT);
//				if (data_array.length != COMPANY_PROFILE_ENTRY_FIELD_SIZE)
//				{
//					CmnLogger.format_error("Incorrect config format: %s", buf);
...
// Open the file
		short ret = CmnDef.RET_SUCCESS;
//		BufferedReader reader = null;
//		String conf_filepath = String.format("%s/%s/%s", CmnDef.get_current_path(), CmnDef.CONF_FOLDERNAME, CmnDef.COMPANY_GROUP_CONF_FILENAME);
//		CmnLogger.format_debug("Try to parse the configuration in %s", conf_filepath);
...
//					continue;
//// Check if the source type in the config file is in order
//				String data_array[] = buf.split(CmnDef.SPACE_DATA_SPLIT);
