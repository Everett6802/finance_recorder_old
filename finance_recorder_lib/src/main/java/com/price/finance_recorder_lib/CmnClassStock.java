...
import com.price.finance_recorder_lib.CmnClass.QuerySet;
...
public class CmnClassStock
{
	static class CompanyGroupSet implements Iterable<Map.Entry<Integer, ArrayList<String>>>
	{
		...
		private static void init_whole_company_number_in_group_map()
		{
			...
			for (int i = 0; i < company_group_size; i++)
			{
				...
				for (ArrayList<String> entry : traverse_entry)
					company_number_array
							.add(entry.get(CompanyProfile.COMPANY_PROFILE_ENTRY_FIELD_INDEX_COMPANY_CODE_NUMBER));
		...
		public Iterator<Map.Entry<Integer, ArrayList<String>>> iterator()
		{
			...
			{
				private Iterator<Map.Entry<Integer, ArrayList<String>>> iter = altered_company_number_in_group_map
						.entrySet().iterator();
		...
		public short add_company_in_group_list(int company_group_number,
				List<String> company_code_number_in_group_array)
		{
			if (is_add_done)
			{
			...
			for (String company_code_number : company_code_number_in_group_array)
			{
				...
				{
					CmnLogger.format_warn("The company code number[%s] has already been added to the group[%d]",
		...
		public short add_company_group(int company_group_number)
		{
			...
			{
				if (company_number_in_group_map.get(company_group_number) != null)
					CmnLogger.format_warn("Select all company group[%d], ignore the original settings......",
		...
		final ArrayList<String> get_company_number_in_group_list(int company_group_index)
		{
			...
			{
				String errmsg = String.format("The company group index[%d] is NOT found in data structure",
						company_group_index);
				CmnLogger.error(errmsg);
		...
		private void setup_for_traverse()
		{
			...
			else
			{
				...
				for (Map.Entry<Integer, ArrayList<String>> entry : company_number_in_group_map.entrySet())
					altered_company_number_in_group_map.put(entry.getKey(), (entry.getValue() != null)
							? entry.getValue() : whole_company_number_in_group_map.get(entry.getKey()));
			}
		}
		...
		public ArrayList<CompanyGroupSet> get_sub_company_group_set_list(int sub_company_group_set_amount)
		{
			...
			if (get_company_amount() <= sub_company_group_set_amount)
			{
				...
				CmnLogger.format_debug(
						"The company amount is less than sub company group set amount. Set the sub company group set amount to %d",
						get_company_amount());
			} else
			{
			...
			for (Map.Entry<Integer, ArrayList<String>> entry : altered_company_number_in_group_map.entrySet())
			{
				...
				for (String company_code_number : company_code_number_list)
				{
					if (sub_company_cnt == 0)
					{
						...
						sub_company_amount_in_group_threshold = (sub_company_group_cnt < rest_company_amount
								? (sub_company_amount + 1) : sub_company_amount);
	...
	public class StockQuerySet extends QuerySet
	{
		...
		public short add_company_list(int company_group_number,
				final ArrayList<String> company_code_number_in_group_array)
		{
			return company_group_set.add_company_in_group_list(company_group_number,
